...
const SearchResultVideo = ({video}) => {
  ...
  return (
    <Link to={`/video/${video?.videoId}`}>
      ...
      { hover!== video?.videoId && <img
            className='h-full w-full object-cover'
            src={video?.thumbnails?.[0]?.url}
            alt=''
            onMouseOver={()=>{handleHover(video?.videoId)}}
        />}
        <ReactPlayer
           ...
           playing={hover===video?.videoId?true:false}
         />
        {video?.lengthSeconds && (
        ...
        <div className="flex flex-col ml-4 md:ml-6 mt-4 md:mt-0 overflow-hidden">
          ...
          <div className="hidden md:flex items-center">
            <div className="flex items-start mr-3">
                    ...
                    <span className="empty:hidden text-sm line-clamp-1 md:line-clamp-1 text-white/[0.7] md:pr-24 md:my-4">
